---
title: "2025 NWSL Schedule Exploration"
author: "ajsportstat"
date: '2025-01-22'
...
difficulty <- ggplot(all_sched, aes(x = Week, y = Placement, group = Team, color = team_color)) +
  geom_line(linewidth = 1) + 
  scale_color_manual(values = color_mapping$team_color, labels = color_mapping$Team) + 
  scale_y_reverse() + 
...
times <- timedf %>% ggplot(aes(fakedate_time, colour = Team)) +
...
times_home_away <- timedf %>% ggplot(aes(fakedate_time, colour=`Home or Away`)) +
  geom_freqpoly(binwidth = 1800) +  # bins of 1800 seconds = 30 minutes
